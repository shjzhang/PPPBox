                              ...
                              int   precision )
{
   ...
   if( useNEU )
   {
      ...
      outfile << solver.getAmbFixedNumL2() << "  ";       // dLat  - #17
...
//    }  // End of 'while ( (updxFile = confReader.fetchListValue( ... "
      ...
      interpCorr.addInterpType(TypeID::corrP2);
      interpCorr.addInterpType(TypeID::corrL1);
      interpCorr.addInterpType(TypeID::corrL2);
...
//    pList.push_back(deltaCorr);       // Add to processing list
      ...
      if(usingC1)
      {
         linear5.addLinear(comb.c1Prefit);
      }
      else
      {
         linear5.addLinear(comb.p1Prefit);
      }
      linear5.addLinear(comb.p2Prefit);
      linear5.addLinear(comb.l1Prefit);
      linear5.addLinear(comb.l2Prefit);
...
//       gdsMap.addGnssRinex(gRin);
          ...
          ++it)
      {
         ...
         // Let's open the output file
      string ttffL2Name(confReader.getValue( "ttffL2File", station ) );
      ofstream ttffL2File;
      ttffL2File.open( ttffL2Name.c_str(), ios::out );
      for(std::vector<double>::iterator it=ttffL2.begin();
          it!=ttffL2.end();
          ++it)
      {
         ttffL2File << (*it) << endl;
