...
//  Revision
//
//  2014/07/06  Modify from ppprtkx22: 
//              - The correction data will is loaded.
//              -'InterpCorrection' is used to interpolate the corrections.
//
//  2014/07/06  Remove the SolverRTKX2FB, because the RTK is only forward 
//              in real-time.
//
//  2014/10/23  Add 'CycleSlipSimulation', which will introduce 'interrupts'
//              in the gnssRinex, which will cause the 're-initialize' and
//              'initialize' of the PPP-RTK.
//
//
//============================================================================
...
class ppprtkx2 : public BasicFramework
{
public:
...
}; // End of 'ppprtkx2' class declaration



   // Let's implement constructor details
ppprtkx2::ppprtkx2(char* arg0)
   :
   BasicFramework(  arg0,
...
"Please consult the default configuration file, 'ppprtkx2.conf', for\n"
"further details.\n\n"
...
"17) VDOP\n" ),
   ...
   confFile( CommandOption::stdType,
             'c',
             "conffile",
   " [-c|--conffile]    Name of configuration file ('ppprtkx2.conf' by default).",
             false )
{
...
}  // End of 'ppprtkx2::ppprtkx2'



   // Method to print solution values
void ppprtkx2::printSolution( ofstream& outfile,
                              const SolverRTKX2& solver,
                              const CommonTime& time,
                              const ComputeDOP& cDOP,
                              bool  useNEU,
                              ...
                              int   precision )
{
   ...
   if( useNEU )
   {
      ...
      outfile << solver.getAmbFixedNumWL() << "  ";       // dLat  - #17
      outfile << solver.getAmbFixedNumL1() << "  ";       // dLon  - #18
...
}  // End of method 'ppprtkx2::printSolution()'



   // Method to print model values
void ppprtkx2::printModel( ofstream& modelfile,
                           const gnssRinex& gData,
                           int   precision )
{
...
}  // End of method 'ppprtkx2::printModel()'
...
void ppprtkx2::spinUp()
{
   ...
   else
   {
      ...
      try
      {
         ...
         confReader.open( "ppprtkx2.conf" );
      }
      catch(...)
      {

         cerr << "Problem opening default configuration file 'ppprtkx2.conf'"
              << endl;
...
}  // End of method 'ppprtkx2::spinUp()'



   // Method that will really process information
void ppprtkx2::process()
{
      //
...
//    }  // End of 'while ( (updxFile = confReader.fetchListValue( ... "
      ...
      if(usingC1)
      {
         interpCorr.setInterpType(TypeID::corrC1);
      }
      else
      {
         interpCorr.setInterpType(TypeID::corrP1);
      }
      interpCorr.addInterpType(TypeID::corrPC);
      interpCorr.addInterpType(TypeID::corrPdelta);
      interpCorr.addInterpType(TypeID::corrL1);
...
//    ComputeDeltaCorrection deltaCorr(TypeID::wetTropo, gdsMap);
//    deltaCorr.addInterpType(TypeID::ionoL1);
//    pList.push_back(deltaCorr);       // Add to processing list
      ...
      {
            // Reconfigure solver
         rtkSolver.setCoordinatesModel(&wnM);
      }

         // Ionospheric stochastic model
      IonoRandomWalkModel ionoModel;
      ionoModel.setInsertInterrupt(true);
      ionoModel.setInitialEpoch(initialTime);
      rtkSolver.setIonosphericModel(&ionoModel);

      double initialIonoVar( confReader.getValueAsDouble( "initialIonoVar",
                             station ) );
      double initialTropVar( confReader.getValueAsDouble( "initialTropVar",
                             station ) );

      rtkSolver.setInitialIonoVar(initialIonoVar);
      rtkSolver.setInitialTropVar(initialTropVar);

         // Add solver to processing list
      pList.push_back(rtkSolver);
...
//         // Whehter this station will be set as peer station ?
//      if( confReader.getValueAsBoolean( "peerStation", station ) )
//      {
//          staticFlag = true;
//      }
//
...
//           // Set the station as 'peer station'
//        gRin.header.source.staticFlag = staticFlag;  
//        cout << "staticFlag" << gRin.header.source.staticFlag << endl;
...
//       gdsMap.addGnssRinex(gRin);
      ...
      ttffWL = rtkSolver.getTTFFWL(); 
      ttffL1 = rtkSolver.getTTFFL1();

         // Let's open the output file
      string ttffWLName(confReader.getValue( "ttffWLFile", station ) );
          ...
          ++it)
      {
         ...
         // Let's open the output file
      string ttffL1Name(confReader.getValue( "ttffL1File", station ) );
      ...
      stime = rtkSolver.getSTime(); 
         // Let's open the output file
      string stimeName(confReader.getValue( "stimeFile", station ) );
      ofstream stimeFile;
      ...
      ttsc = rtkSolver.getTTSC(); 
         // Let's open the output file
      string ttscName(confReader.getValue( "ttscFile", station ) );
      ofstream ttscFile;
      ttscFile.open( ttscName.c_str(), ios::out );
      for(std::vector<double>::iterator it=ttsc.begin();
          it!=ttsc.end();
          ++it)
      {
         ttscFile << (*it) << endl;
