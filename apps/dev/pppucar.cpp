...
class pppucar : public BasicFramework
{
public:
...
}; // End of 'pppucar' class declaration



   // Let's implement constructor details
pppucar::pppucar(char* arg0)
   :
   BasicFramework(  arg0,
...
"Please consult the default configuration file, 'pppucar.conf', for\n"
"further details.\n\n"
...
"\n10) PDOP\n"),
   ...
   confFile( CommandOption::stdType,
             'c',
             "conffile",
   "Name of configuration file ('pppucar.conf' by default).",
             false ),
...
}  // End of 'pppucar::pppucar'



   // Method to print solution values
void pppucar::printSolution( ofstream& outfile,
                              const SolverPPPUCAR& solver,
                              const CommonTime& time,
                              const ComputeDOP& cDOP,
                              bool  useNEU,
...
}  // End of method 'pppucar::printSolution()'



   // Method to print model values
void pppucar::printModel( ofstream& modelfile,
                        const gnssRinex& gData,
                        int   precision )
{
...
}  // End of method 'pppucar::printModel()'
...
void pppucar::spinUp()
{
   ...
   else
   {
      ...
      try
      {
         ...
         confReader.open( "pppucar.conf" );
      }
      catch(...)
      {

         cerr << "Problem opening default configuration file 'pppucar.conf'"
              << endl;
...
}  // End of method 'pppucar::spinUp()'



   // Method that will really process information
void pppucar::process()
{
...
//       }
   ...
   while( rnxit != rnxFileListVec.end() )
   {
      ...
      SolverPPPUCAR   pppucarSolver(isNEU);
      SolverPPPUCARFB fbpppucarSolver(isNEU);

      if(debugLevel)
      {
         pppucarSolver.debugLevel = debugLevel;
         fbpppucarSolver.debugLevel = debugLevel;
      }
      ...
      if ( cycles > 0 )
      {
         ...
         {
               // Reconfigure solver
            fbpppucarSolver.setCoordinatesModel(&wnM);
         }
         ...
         if(ambMethod==1)
         {
            fbpppucarSolver.setARMethod("BootStrapping");
         }
         else if(ambMethod==2)
         {
         ...
         pList.push_back(fbpppucarSolver);
      ...
      else
      {
         ...
         {
               // Reconfigure solver
            pppucarSolver.setCoordinatesModel(&wnM);
         }
         ...
         if(ambMethod==1)
         {
            pppucarSolver.setARMethod("BootStrapping");
         }
         else if(ambMethod==2)
         {
      ...
      if ( cycles < 1 )
      {
         ...
         ttffWL = pppucarSolver.getTTFFWL(); 
         ttffL1 = pppucarSolver.getTTFFL1();
      ...
      try
      {
         ...
         fbpppucarSolver.ReProcess(cycles);
      ...
      while( fbpppucarSolver.LastProcess(gRin) )
      {
         CommonTime time(gRin.header.epoch);
      ...
      }  // End of 'while( fbpppucarSolver.LastProcess(gRin) )'
      ...
      std::vector<double> ttffWL;
      std::vector<double> ttffL1;
      ttffWL = fbpppucarSolver.getTTFFWL(); 
      ttffL1 = fbpppucarSolver.getTTFFL1();
      ...
      std::vector<double> stime;
      stime = fbpppucarSolver.getSTime(); 
         // Let's open the output file
      string stimeName( outputFileName+ ".restart");
      ofstream stimeFile;
      stimeFile.open( stimeName.c_str(), ios::out );
