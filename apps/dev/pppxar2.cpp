...
//  Revision
//
//  - Fix the widelane and narrowlane ambiguities
//
//  - Store nominalPos into source
//
//    @code
//     gRin.header.source.nominalPos = nominalPos;
//    @endcode
//
//  - PPP ambiguity resolution with Lw/L3 model
//    by shjzhang, 2014/07/15
//
//============================================================================
...
#include "ComputeElevWeights.hpp"
   ...
   // Class to compute the code biases
...
class pppxar : public BasicFramework
{
public:
...
}; // End of 'pppxar' class declaration



   // Let's implement constructor details
pppxar::pppxar(char* arg0)
   :
   BasicFramework(  arg0,
...
"Please consult the default configuration file, 'pppxar.conf', for\n"
"further details.\n\n"
...
"17) VDOP\n" ),
   ...
   confFile( CommandOption::stdType,
             'c',
             "conffile",
   " [-c|--conffile]    Name of configuration file ('pppxar.conf' by default).",
             false )
{
...
}  // End of 'pppxar::pppxar'



   // Method to print solution values
void pppxar::printSolution( ofstream& outfile,
                              const SolverPPPXAR2& solver,
                              const CommonTime& time,
                              const ComputeDOP& cDOP,
                              bool  useNEU,
                              ...
                              int   precision )
{
   ...
   if( useNEU )
   {
      ...
      outfile << solver.getAmbFixedNumNL() << "  ";       // dLon  - #18
...
}  // End of method 'pppxar::printSolution()'



   // Method to print model values
void pppxar::printModel( ofstream& modelfile,
                           const gnssRinex& gData,
                           int   precision )
{
...
}  // End of method 'pppxar::printModel()'
...
void pppxar::spinUp()
{
   ...
   else
   {
      ...
      try
      {
         ...
         confReader.open( "pppxar.conf" );
      }
      catch(...)
      {

         cerr << "Problem opening default configuration file 'pppxar.conf'"
              << endl;
...
void pppxar::process()
{
   ...
   while ( (station = confReader.getEachSection()) != "" )
   {
      ...
      if(useRinexClock)
      {
         ...
         // Declare a "IonexStore" object to ionex file
      IonexStore ionexStore;

         // Load all the EOP files from variable list
      string ionexFile;
      while ( ( ionexFile = confReader.fetchListValue("IonexFileList", station ) ) != "" )
      {
         try
         {
            ionexStore.loadFile( ionexFile );
         }
         catch (...)
         {
               // If file doesn't exist, issue a warning
            cerr << "EOP file '" << ionexFile << "' doesn't exist or you don't "
                 << "have permission to read it. Skipping it." << endl;
...
//    CorrectUPDXs updxCorr(updxStore);
//    pList.push_back(updxCorr);
      ...
      markArc.setUnstablePeriod(900.0); // 15 min
      ...
      pList.push_back(computeTropo);       // Add to processing list
         ...
         // Object to compute the differential code biases between P1 and P2, P1 and C1
      CorrectCodeBiases dcbCorr;
      dcbCorr.setUsingC1( usingC1 );
      dcbCorr.setDCBFile( confReader.getValue( "p1p2File", station ),
                          confReader.getValue( "p1c1File", station ) );
      pList.push_back(dcbCorr);


         // Object to compute the ionospheric delay and related parameters
         // Warning:
         // The DCB values have been corrected with 'CorrectCodeBiases', so
         // here you should set 'useDCB' as false.
      IonexModel computeIono(nominalPos, 
                             ionexStore);
      computeIono.setUseDCB(false);
      pList.push_back(computeIono);       // Add to processing list
      ...
      SolverPPPXAR2   pppxarSolver(isNEU);
      pppxarSolver.setFixWithLambda(false);

//    SolverPPPXA2RFB fbpppxarSolver(isNEU);
...
//       {
//             // Reconfigure solver
//          fbpppxarSolver.setCoordinatesModel(&wnM);
//       }

//          // Add solver to processing list
//       pList.push_back(fbpppxarSolver);
      ...
      else
      {
         ...
         {
               // Reconfigure solver
            pppxarSolver.setCoordinatesModel(&wnM);
         }

            // Add solver to processing list
         pList.push_back(pppxarSolver);
      ...
      if( printmodel )
      {
         ...
         // Whehter this station will be set as static station ?
      ...
      while(rin >> gRin)
      {
         ...
         if ( cycles < 1 )
         {
            ...
            // Set the station as 'reference station'
...
//       fbpppxarSolver.ReProcess(cycles);
...
//    while( fbpppxarSolver.LastProcess(gRin) )
//    {
...
//    }  // End of 'while( fbpppxarSolver.LastProcess(gRin) )'
...
}  // End of 'pppxar::process()'


   // Method that will be executed AFTER the 'Process'
void pppxar::shutDown()
{

      // Define the file name to store the 'gdsMap'
   string correctionFile=  confReader.getValue( "correctionFile", "DEFAULT" );

      // Warning, the 'gdsMap' to a binary file 
   saveGnssDataMap( gdsMap, correctionFile);
