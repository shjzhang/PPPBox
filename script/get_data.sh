...
Help()
{
	...
	echo "                                  产品类型选项：COD_EPH, IGS_ATX_08,等"
...
URLMatch()
{
	# typeid with format "AGENCY_TYPE", such as "COD_CLK", "COD_EPH" 
	str="$typeid"


	# Locate the str in the given file
	if [[ -n u ]] && [[ -f $u ]]
	then
		loc=$(grep -w $str $u) 
	else
	...
	then
      ...
      echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      echo "The data type ($typeid) can't be found in in the URL_List.txt  !"
      echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
		exit 1
	fi
...
Download()
{
   ...
   if [[ -e $savefile ]] || [[ -e $download_file ]]
	then
      echo "The file $savefile or $download_file already exists!"
	else
...
MakeList()
{
	...
	then
	   ...
	   if [[ $data_type = "OBS" ]] && [[ -e "$project.rnxlist" ]]
	   then
	   	rm "$project.rnxlist"
	   fi

	   if [[ $data_type = "EPH" ]] && [[ -e "$project.ephlist" ]]
	   then
	   	rm "$project.ephlist"
	   fi
	   if [[ $data_type = "ERP" ]] && [[ -e "$project.erplist" ]]
	   then
	   	rm "$project.erplist"
	   fi
	   if [[ $data_type = "ATX" ]] && [[ -e "$project.atxlist" ]]
	   then
	   	rm "$project.atxlist"
	   fi
	   if [[ $data_type = "CLK" ]] && [[ -e "$project.clklist" ]]
	   then
	   	rm "$project.clklist"
	   fi
	   if [[ $data_type = "TEC" ]] && [[ -e "$project.teclist" ]]
	   then
	   	rm "$project.teclist"
	   fi
	   if [[ $data_type = "POS" ]] && [[ -e "$project.snxlist" ]]
	   then
	   	rm "$project.snxlist"
	   fi
	   if [[ $data_type = "NAV" ]] && [[ -e "$project.navlist" ]]
	   then
	   	rm "$project.navlist"
	   fi
	   if [[ $data_type = "SSC" ]] && [[ -e "$project.ssclist" ]]
	   then
	   	rm "$project.ssclist"
	   fi
	   if [[ $data_type = "DCB" ]] && [[ -e "$project.dcblist" ]]
	   then
	   	rm "$project.dcblist"
	   fi
	   if [[ $data_type = "REC" ]] && [[ -e "$project.reclist" ]]
	   then
	   	rm "$project.reclist"
	   fi
...
while getopts "b:e:i:t:u:s:p:l:dmh" arg
do
	case $arg in
		b)  b=$OPTARG;;
		e)  e=$OPTARG;;
...
# now, Let's get the data_type from the 'typeid'
data_type=`echo $typeid | awk -F '_' '{print $2}'`
...
if [[ $data_type = "EPH" ]] || [[ $data_type = "CLK" ]] || [[ $data_type = "CLK_30S" ]]
then
...
if [[ $data_type = "ERP" ]] || [[ $data_type = "SSC" ]]
then
...
then # When the product type is OBS and 's' displays a list file.
   ...
   exit 1 
	...
	do
      ...
      if [[ ! -e $savefile ]] && [[ ! -e $download_file ]]
	   then
		...
		if [[ $data_type = "OBS" ]] && [[ $segment_intv -lt 24 ]]
		then
			if [ $de = 1 ]
			then
				Segment 
	...
	if [[ $data_type = "OBS" ]] && [[ $merge = 1 ]] && [[ $segment_intv -ge 24 ]]
	then
		Merge
		FileList=("${MergeFile[@]}")
		MakeList
