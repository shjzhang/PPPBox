...
Help()
{
	...
	echo "    : bash get_eph.sh -b \"2014 3 1 0 0 0\" -e \"2014 3 3 0 0 0\" -i 24 -a IGS -t \"data_type.lis\" -u \"url_list.txt\" -l igs1781 -p \"./data\" "
	exit 1
}
...
DataDownload()
{
	  ...
	  # data_type=("EPH" "CLK_30S" "ATX_08" "ERP")
	...
	do
		...
		if [[ -n $prefix ]]
		then
			bash get_data.sh -b "$b" -e "$e" -i $intv -a $agency -t $var -u $u -l $prefix -p $savepath -d 
		else
			bash get_data.sh -b "$b" -e "$e" -i $intv -a $agency -t $var -u $u -p $savepath -d 
		fi
   done	
}
