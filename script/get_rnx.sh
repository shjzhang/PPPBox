...
#
# WARNING:
# We will call the get_data.sh to download the observation data,
# which published by the IGS and other agency. 
#
# Copyright (c)  
...
Help()
{
	...
	echo "    -m,   --merge the observation     将观测值文件进行合并，合并后文件名由用户提供
	                  file                       "
	echo "示例: bash get_rnx.sh -b \"2014 3 1 0 0 0\" -e \"2014 3 3 0 0 0\" -i 24 -a IGS -s \"staion_list.txt\" -u \"url_list.txt\" -p \"./data\" "
	echo "      bash get_rnx.sh -b \"2014 3 1 0 0 0\" -e \"2014 3 3 0 0 0\" -i 24 -a IGS -s \"staion_list.txt\" -u \"url_list.txt\" -p \"./data\" -l igs1781"
	exit 1
}
