...
Help()
{
   ...
   echo "-t,   --data type                 数据类型（参见url.list）"
...
DataDownload()
{
	...
	then
      echo $data_type
		bash get_data.sh -b "$b" -e "$e" -i $intv -t $data_type -s "$s" -u $u -l $prefix -p $savepath -d -m
	else
		bash get_data.sh -b "$b" -e "$e" -i $intv -t $data_type -s "$s" -u $u -l $prefix -p $savepath -d 
	fi
}

# Read the input arguments
while getopts "b:e:i:u:t:s:p:l:mh" arg
do
	case $arg in
		b)  b=$OPTARG;;
		e)	e=$OPTARG;;
