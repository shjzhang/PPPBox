...
void IonoDCB::process()
{
   ...
   vector<string>::const_iterator outit = outputFileListVec.begin();
      // to store the information of plural stations
      // used in class SolverIonoDCB
   gnssDataMap gData;
   ...
   while( rnxit != rnxFileListVec.end() )
   {
     ...
     // cout<<"The position of antenna is : "<<antennaPos<<endl;
     ...
     IonexModel ionex(nominalPos, IonexMapList);
	  // set the Type of mapping function(same with CODE) 
	 ionex.setIonoMapType("MSLM");
       ...
       try
         {
            ...
            cout << gData.getValue(time,source,sat,TypeID::L1)<<" ";
            cout << gData.getValue(time,source,sat,TypeID::L2)<<" ";
            cout << endl;
  ...
  /*(for (gnssDataMap::const_iterator it= gData.begin();
       it!= gData.end(); )
   {
     ...
     if ((static_cast<int>(second)%720)==0)
     {
      cout<<it->first<<endl;	   
