...
namespace gpstk
{
   ...
   Rinex3NavData2::Rinex3NavData2(const RinexNavData& rnd)
   {
      // Epoch
      time = rnd.time;
      satSys = "G";
   ...
   // Private helper routine for constructors from OrbitEph2-based Ephemerides
   void Rinex3NavData2::loadFrom(const OrbitEph2 *oeptr)
   {
      time = oeptr->ctToc;
   ...
   // Initializes the nav data with a GPSEphemeris2
   Rinex3NavData2::Rinex3NavData2(const GPSEphemeris2& gpseph)
   {
      loadFrom(dynamic_cast<const OrbitEph2*>(&gpseph));
   ...
   Rinex3NavData2::Rinex3NavData2(const GalEphemeris& galeph)
   {
      loadFrom(dynamic_cast<const OrbitEph2*>(&galeph));
   ...
   Rinex3NavData2::Rinex3NavData2(const BDSEphemeris& bdseph)
   {
      loadFrom(dynamic_cast<const OrbitEph2*>(&bdseph));
   ...
   Rinex3NavData2::Rinex3NavData2(const QZSEphemeris& qzseph)
   {
      loadFrom(dynamic_cast<const OrbitEph2*>(&qzseph));
   ...
   Rinex3NavData2::Rinex3NavData2(const EngEphemeris& ee) // GPS only
   {
      // epoch info
      satSys = ee.getSatSys();
      PRNID  = ee.getPRNID();
   ...
   }  // End of 'Rinex3NavData2::Rinex3NavData2(const EngEphemeris& ee)'

      // This constructor initializes R3NavData with Glonass data.
   Rinex3NavData2::Rinex3NavData2(const GloEphemeris& gloe)
   {
   ...
   }  // End of 'Rinex3NavData2::Rinex3NavData2(const GloEphemeris& ge)'
       ...
       */
   void Rinex3NavData2::reallyGetRecord(FFStream& ffs)
   ...
   }  // End of method 'Rinex3NavData2::reallyGetRecord(FFStream& ffs)'


      // Outputs the record to the FFStream \a s.
   void Rinex3NavData2::reallyPutRecord(FFStream& ffs) const
      throw(exception, FFStreamError, StringException)
   {
   ...
   }  // End of method 'Rinex3NavData2::reallyPutRecord(FFStream& ffs)'

      // A debug output function.
      // Prints the PRN id and the IODC for this record.
   void Rinex3NavData2::dump(ostream& s) const
   {
      s << "Rinex3NavData2 dump: "
   ...
   string Rinex3NavData2::dumpString(void) const
   {
      ostringstream s;
      s << "RND " << satSys
   ...
   }  // End of method 'Rinex3NavData2::asString

      // Deprecated; use GPSEphemeris2.
      // Converts this Rinex3NavData2 to an EngEphemeris object.
   Rinex3NavData2::operator EngEphemeris() const throw()
   {
      EngEphemeris ee;

      // There's no TLM word in Rinex3NavData2, so it's set to 0.
   ...
   }  // End of 'Rinex3NavData2::operator EngEphemeris()'

   // Private helper routine for casts from this to OrbitEph2-based Ephemerides
   void Rinex3NavData2::castTo(OrbitEph2 *oeptr) const
   {
      try {
      ...
      // Casts Rinex3NavData2 to a GPSEphemeris2 object.
   Rinex3NavData2::operator GPSEphemeris2() const throw()
   {
      GPSEphemeris2 gpse;

      // fill the OrbitEph2 parts
      OrbitEph2* ptr = dynamic_cast<OrbitEph2*>(&gpse);
      ...
      // Casts this Rinex3NavData2 to a GloEphemeris object.
   Rinex3NavData2::operator GloEphemeris() const throw()
   {
   ...
   }  // End of 'Rinex3NavData2::operator GloEphemeris()'

      // Casts Rinex3NavData2 to a GalEphemeris object.
   Rinex3NavData2::operator GalEphemeris() const throw()
   {
      GalEphemeris gale;

      // fill the OrbitEph2 parts
      OrbitEph2 *ptr = dynamic_cast<OrbitEph2*>(&gale);
      castTo(ptr);                                          // sets dataLoadedFlag
      ...
      // Casts Rinex3NavData2 to a BDSEphemeris object.
   Rinex3NavData2::operator BDSEphemeris() const throw()
   {
      BDSEphemeris bdse;

      // fill the OrbitEph2 parts
      OrbitEph2* ptr = dynamic_cast<OrbitEph2*>(&bdse);
      ...
      // Casts Rinex3NavData2 to a QZSEphemeris object.
   Rinex3NavData2::operator QZSEphemeris() const throw()
   {
      QZSEphemeris qzse;

      // fill the OrbitEph2 parts
      castTo(dynamic_cast<OrbitEph2*>(&qzse));
   ...
   list<double> Rinex3NavData2::toList() const
   {
   ...
   }  // End of method 'Rinex3NavData2::toList()'


      /* Generates the PRN/epoch line and outputs it to strm
       *  @param strm RINEX Nav stream
       */
   void Rinex3NavData2::putPRNEpoch(Rinex3NavStream& strm) const
   ...
   }  // End of 'Rinex3NavData2::putPRNEpoch(Rinex3NavStream& strm)'
   ...
   void Rinex3NavData2::putRecord(const int& nline, Rinex3NavStream& strm) const
      throw(StringException, FFStreamError)
   {
      ...
      try {
         ...
         // SBAS and GLO end here
            ...
            // Internally (Rinex3NavData2), weeknum=week of HOW
   ...
   }  // End of method 'Rinex3NavData2::putRecord(const int& nline,...'


   void Rinex3NavData2::getPRNEpoch(Rinex3NavStream& strm)
      throw(StringException, FFStreamError)
   {
      try {
         int i;
   ...
   void Rinex3NavData2::getRecord(const int& nline, Rinex3NavStream& strm)
      throw(StringException, FFStreamError)
   {
      if(nline < 1 || nline > 7) {
         FFStreamError fse(string("Invalid line number ") + asString(nline));
      ...
      try {
         ...
         if(nline == 1) {
            ...
            // Internally (Rinex3NavData2), weeknum is week of HOW
            if(HOWtime - Toe > HALFWEEK)
               weeknum--;
            else if(HOWtime - Toe < -HALFWEEK)
               weeknum++;
