/// @file OrbitEph2.hpp Encapsulates the "least common denominator" orbit parameters
...
// Modification
//
// Q.Liu, Wuhan University, 2017-01-03   Add the members for real-time ephmeris
//                                       data.
//=============================================================================

#ifndef GPSTK_OrbitEph2_HPP
#define GPSTK_OrbitEph2_HPP
...
namespace gpstk
{
   ...
   public:
	   /// Default constuctor
      OrbitEph2(void) : dataLoadedFlag(false), dndot(0.0), Adot(0.0),
                       beginValid(CommonTime::END_OF_TIME),
                       endValid(CommonTime::BEGINNING_OF_TIME),
                       checkState(unchecked),orbCorr(0),clkCorr(0)
         { }

      /// Destructor
      virtual ~OrbitEph2(void);
      ...
      virtual std::string getName(void) const
         { return std::string("OrbitEph2"); }

      /// This function returns the health status of the SV. OrbitEph2 has no health
      ...
      /// In OrbitEph2 it simply assumes a 4-hour fit interval; however the derived
      ...
      /// The ephmeris ckeck state type.
      enum e_checkState {unchecked, ok, bad, outdated};

      /// Set ephmeris ckeck state.
      void setCheckState(e_checkState state) {checkState = state;}

      /// Get ephmeris ckeck state.
      e_checkState getCheckState() const{return checkState;}

      /// Set the orbit correction
   ...
   /// Write OrbitEph2 to output stream
   std::ostream& operator<<(std::ostream& os, const OrbitEph2& eph);

} // end namespace

#endif // GPSTK_OrbitEph2_HPP
