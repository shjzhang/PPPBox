...
{
   //---------------------------------------------------------------------------------
   Xvt OrbitEphStore2::getXvt(const SatID& sat, const CommonTime& t) const
   {
      try {
   ...
   //---------------------------------------------------------------------------------
   void OrbitEphStore2::dump(ostream& os, short detail) const
   {
   ...
   }  // end OrbitEph* OrbitEphStore2::addEphemeris(const OrbitEph* eph)

   //---------------------------------------------------------------------------------
   void OrbitEphStore2::edit(const CommonTime& tmin, const CommonTime& tmax)
   {
   ...
   //---------------------------------------------------------------------------------
   unsigned OrbitEphStore2::size(void) const
   {
      unsigned counter = 0;
      SatTableMap::const_iterator it;
      for(it = satTables.begin(); it != satTables.end(); it++)
   ...
   //---------------------------------------------------------------------------------
   unsigned OrbitEphStore2::size(const SatID& sat) const
   {
      unsigned n(0);
      SatTableMap::const_iterator it;
      if(sat.id == -1) {
   ...
   const OrbitEph2* OrbitEphStore2::findUserOrbitEph(const SatID& sat,
   ...
   }  // end OrbitEph* OrbitEphStore2::findUserOrbitEph


   //---------------------------------------------------------------------------------
   const OrbitEph2* OrbitEphStore2::findNearOrbitEph(const SatID& sat,
                                                   const CommonTime& t) const
   {
   ...
   int OrbitEphStore2::addToList(list<OrbitEph2*>& v, SatID sat) const
   {
      int n = 0;
      SatTableMap::const_iterator it;
      for (it = satTables.begin(); it != satTables.end(); it++)
   ...
   //---------------------------------------------------------------------------------
   const OrbitEphStore2::TimeOrbitEphTable&
      OrbitEphStore2::getTimeOrbitEphMap(const SatID& sat) const
   {
   ...
   //---------------------------------------------------------------------------------
   const string OrbitEphStore2::fmt("%Y/%02m/%02d %02H:%02M:%02S %P");
