...
// Modification
//
// Support the real-time stream process.    Q.Liu   2017/02/24
//
//=============================================================================

#ifndef GPSTK_ORBITEPHSTORE2_HPP
#define GPSTK_ORBITEPHSTORE2_HPP
...
namespace gpstk
{
   ...
   class OrbitEphStore2 : public XvtStore<SatID>
   {
   public:
      ...
      /// Destructor
      virtual ~OrbitEphStore2() { clear(); }

      /// Return a string that will identify the derived class
      virtual std::string getName(void) const
         { return std::string("OrbitEphStore2"); }
...
//TD make work for sat = -1, system
      ...
      virtual OrbitEph2* addEphemeris(const OrbitEph2* eph);
      ...
      /// Explanation of find() function for OrbitEphStore2
      ...
      virtual const OrbitEph2* findUserOrbitEph(const SatID& sat, const CommonTime& t)
         const;
      ...
      /// @return a pointer to the desired OrbitEph, or NULL if no OrbitEph found.
      virtual const OrbitEph2* findNearOrbitEph(const SatID& sat, const CommonTime& t)
         const;
      ...
      /// @param t the time of interest
      /// @return a pointer to the desired OrbitEph, or NULL if no OrbitEph found.
      const OrbitEph2* findOrbitEph(const SatID& sat, const CommonTime& t) const
      {
      ...
      virtual int addToList(std::list<OrbitEph2*>& v,
      ...
      bool getSatHealth(const SatID& sat, const CommonTime& t) const
      {
         ...
         const OrbitEph2 *eph = findOrbitEph(sat,t);
         return eph->isHealthy();
      }

      /// Set the choice if we use correction data
      /// @author Q.Liu
      void setUseCorrection(bool useCorr){useCorrection = useCorr;}
      ...
      typedef std::map<CommonTime, OrbitEph2*> TimeOrbitEphTable;
   ...
   protected:
      ...
      bool useCorrection;     ///< If use the orbit and clock correction

      /// Convenience routines
      void updateTimeLimits(const OrbitEph2* eph)
      {
   ...
   }; // end class OrbitEphStore2

   //@}

} // namespace

#endif // GPSTK_ORBITEPHSTORE2_HPP
