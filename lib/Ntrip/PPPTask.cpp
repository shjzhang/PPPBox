...
bool PPPTask::waitForCorr(const CommonTime &epoTime)
{
    ...
    else if(!m_lastClkCorrTime.valid())
    {
        return true;
    }
    else
...
{
    spinUp();
    string obsFileList = m_confReader.getValue("ObsFileList");
    string erpFile     = m_confReader.getValue("EOPFile");
    string ephFileList = m_confReader.getValue("EphFileList");
    string mscFileName = m_confReader.getValue("MSCFile");
       ...
       // Open eopFileList File
    {
       try
       {
          eopStore.loadIGSFile( erpFile );
       }
       catch (FileMissingException& e)
       {
             // If file doesn't exist, issue a warning
          cerr << "EOP file '" << erpFile << "' doesn't exist or you don't "
               << "have permission to read it. Skipping it." << endl;
       }
    }
       // Close file
    ...
    string dcbFile = m_confReader.getValue( "DCBFile");
   ...
   ifstream rnxFileListStream;
      ...
      // Open eopFileList File
   rnxFileListStream.open(obsFileList.c_str());
   if(!rnxFileListStream)
   {
         // If file doesn't exist, issue a warning
      cerr << "rinex file List Name'" << obsFileList << "' doesn't exist or you don't "
           << "have permission to read it. Skipping it." << endl;
   ...
   {
      cerr << obsFileList  << "obsFileList is empty!! "
           << endl;
   }
...
//      cc2noncc.loadRecTypeFile( m_confReader.getValue("recTypeFile"));
