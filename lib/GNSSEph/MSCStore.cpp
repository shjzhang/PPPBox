...
//-----------------------------------------------------------------------------
   ...
   void MSCStore::loadFile(const string& filename)
      throw(FFStreamError, FileMissingException)
   {
      try
      {
         MSCStream strm(filename.c_str());
            // enable exception throw 
      ...
      {
         FFStreamError e("load file error!");
         GPSTK_THROW(e);
      }
   }
      ...
      const throw(InvalidRequest)
   {
      try
      {
         ...
         MMci mm = mscMap.find( StringUtils::lowerCase(key) );
            ...
            // Find station with upper-case charater
         MMci MM = mscMap.find( StringUtils::upperCase(key) );
         if( MM != mscMap.end() )
         {
            mm = MM;
         }
