...
namespace gpstk
{
   ...
   std::string EclipsedSatMarker::getClassName() const
   { return "EclipsedSatMarker"; }
       ...
       */
   EclipsedSatMarker& EclipsedSatMarker::setConeAngle(const double angle)
   {
   ...
   }  // End of method 'EclipsedSatMarker::setConeAngle()'
       ...
       */
   EclipsedSatMarker& EclipsedSatMarker::setPostShadowPeriod(
                                                         const double pShTime)
   {
   ...
   }  // End of method 'EclipsedSatMarker::setPostShadowPeriod()'
       ...
       */
   satTypeValueMap& EclipsedSatMarker::Process( const CommonTime& epoch,
      ...
      throw(ProcessingException)
   {
      ...
      try
      {
         ...
         for (it = gData.begin(); it != gData.end(); ++it) 
         {
            ...
            {
                  // If satellite is eclipsed, then insert a indicator into the 
                  // data
               (*it).second[TypeID::eclipse] = 1.0;
            ...
            else
            {
               ...
               if( shadowEpoch.find( (*it).first ) != shadowEpoch.end() )
               {
                  ...
                  if( std::abs( ( epoch - shadowEpoch[(*it).first] ) ) <=
                                postShadowPeriod )
                  {
                        // Satellite left shadow, but too recently. Aslo mark it
                     (*it).second[TypeID::eclipse] = 1.0;
   ...
   }  // End of 'EclipsedSatMarker::Process()'
       ...
       * @param gData    Data object holding the data.
       */
   gnssRinex& EclipsedSatMarker::Process(gnssRinex& gData)
      throw(ProcessingException)
   {
