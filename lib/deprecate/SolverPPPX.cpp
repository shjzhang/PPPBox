#pragma ident "$Id: SolverPPPX.cpp 2645 2012-11-05 03:23:24Z shjzhang $"
...
namespace gpstk
{
   ...
   int SolverPPPX::classIndex = 9300000;


      // Returns an index identifying this object.
   int SolverPPPX::getIndex() const
   { return index; }


      // Returns a string identifying this object.
   std::string SolverPPPX::getClassName() const
   { return "SolverPPPX"; }
       ...
       */
   SolverPPPX::SolverPPPX(bool useNEU, bool fixPos)
   ...
   }  // End of 'SolverPPPX::SolverPPPX()'



      // Initializing method.
   void SolverPPPX::Init(void)
   {
       ...
       */
   gnssSatTypeValue& SolverPPPX::Process(gnssSatTypeValue& gData)
      throw(ProcessingException, SVNumException)
   {
   ...
   }  // End of method 'SolverPPPX::Process()'
       ...
       * @param gData     Data object holding the data.
       */
   gnssRinex& SolverPPPX::Process(gnssRinex& gData)
      throw(ProcessingException, SVNumException)
   {
       ...
       * @param gData    Data object holding the data.
       */
   gnssRinex& SolverPPPX::preCompute( gnssRinex& gData )
      throw(ProcessingException, SVNumException)
   {
...
//       cout << "numSat" << currSatSet.size() << endl;
         ...
         numIono = numCurrentSV - 1;
            ...
            // Ionospheric delay constraints: numCurrentSV - 5;
         numMeas = 6 * numCurrentSV + numIono;
         ...
         for( int i=0; i<numCurrentSV; i++ )
         {
            ...
            measVector( i + 4*numCurrentSV ) = prefitPW(i);
            measVector( i + 5*numCurrentSV ) = prefitLW(i);
         }
...
//       cout << "apriIon" << apriIon.size() << endl;
         ...
         for(int i=0; i<numCurrentSV;i++)
         {
             ...
             if( i != imax )
             {
                ...
                measVector(c1 + 6*numCurrentSV) = apriIon(i) - ionoBase;
...
//       cout << "measVector" << measVector << endl;
         ...
         if ( dummy.numSats() == numCurrentSV )
         {
            ...
            for( int i=0; i<numCurrentSV; i++ )
            {
               ...
               rMatrix( i + 5*numCurrentSV, i + 5*numCurrentSV ) 
            ...
            for( int i=0; i<numCurrentSV; i++ )
            {
                if(i != imax)
                {
                   rMatrix(c1 + 6*numCurrentSV, c1 + 6*numCurrentSV) = 1.0 * wVec(i);
                   ++c1;
                }
            }
         ...
         else
         {
            ...
            for( int i=0; i<numCurrentSV; i++ )
            {
                if(i != imax)
                {
                   rMatrix(c1 + 6*numCurrentSV, c1 + 6*numCurrentSV) = 1.0 ;
                   ++c1;
                }
            }
         ...
         for( int i=0; i<numCurrentSV; i++ )
         {
            ...
            for( int j=0; j<numVar; j++ )
            {
               ...
               hMatrix( i + 4*numCurrentSV, j ) = dMatrix(i,j); // PW
               hMatrix( i + 5*numCurrentSV, j ) = dMatrix(i,j); // LW
            }
              ...
              ++itSat )
         {
            ...
            hMatrix( count1 + 4*numCurrentSV, numVar + count1 ) = 1.283333333; // PW
            hMatrix( count1 + 5*numCurrentSV, numVar + count1 ) = 1.283333333; // LW
              ...
              ++itSat )
         {
            ...
            hMatrix( count1 + 2*numCurrentSV, numVar +   numCurrentSV + count1 ) = +0.190293672798;
               // Warnig:
               // B2 = B1 - Bw
            hMatrix( count1 + 3*numCurrentSV, numVar +   numCurrentSV + count1 ) = +0.244210213425;
            hMatrix( count1 + 3*numCurrentSV, numVar + 2*numCurrentSV + count1 ) = -0.244210213425;
               // ambWL are listed after ( srcIndexedTypes + ionoL1 + ambL1 + ambL2)
            hMatrix( count1 + 5*numCurrentSV, numVar + 2*numCurrentSV + count1 ) = +0.861918400322;
         ...
         for(int i=0; i<numCurrentSV;i++)
         {
             ...
             {
                   // Single difference ionospheric delay equation
                hMatrix(count1 + 6*numCurrentSV, numVar + i   ) =  1.0;
                hMatrix(count1 + 6*numCurrentSV, numVar + imax) = -1.0;
   ...
   }  // End of method 'SolverPPPX::preCompute()'
      ...
      //
   gnssRinex& SolverPPPX::Compute( gnssRinex& gData )
      throw(InvalidSolver)
   {
   ...
   }  // End of method 'SolverPPPX::Compute()'
      ...
      // @param gData    Data object holding the data.
      //
   int SolverPPPX::TimeUpdate( const Matrix<double>& phiMatrix,
...
of qMatrix");
   ...
   }  // End of method 'SolverPPPX::TimeUpdate()'
      ...
      // @param gData    Data object holding the data.
      //
   int SolverPPPX::MeasUpdate( const Vector<double>& prefitResiduals,
...
matrix and a priori state estimation vector do not match.");
   ...
   }  // End of method 'SolverPPPX::MeasUpdate()'
       ...
       * @param gData    Data object holding the data.
       */
   gnssRinex& SolverPPPX::postCompute( gnssRinex& gData )
...
//       cout << "var:WL" << covMatrix(numVar + 2*numCurrentSV + 1, numVar+2*numCurrentSV+1) << endl;
         ...
         for( int i=0; i<numCurrentSV; i++ )
         {
            ...
            postfitPW(i) = postfitResiduals( i + 4*numCurrentSV );
            postfitLW(i) = postfitResiduals( i + 5*numCurrentSV );
         }
         ...
         gData.insertTypeIDVector(TypeID::postfitLdelta, postfitLW);
            ...
            // Now, get the ionoL1/BL1/BWL from 'currentState'
...
//       cout << currSatSet.size() << endl;
   ...
   }  // End of method 'SolverPPPX::postCompute()'
       ...
       *
       */
   SolverPPPX& SolverPPPX::setNEU( bool useNEU )
   {
   ...
   }  // End of method 'SolverPPPX::setNEU()'
       ...
       *
       */
   SolverPPPX& SolverPPPX::setFixCoordinate( bool fixPos)
   {
   ...
   }  // End of method 'SolverPPPX::setFixCoordinate'
       ...
       */
   SolverPPPX& SolverPPPX::setCoordinatesModel( StochasticModel* pModel )
   {
   ...
   }  // End of method 'SolverPPPX::setCoordinatesModel()'


      /** Return the CURRENT number of satellite.
       */
   int SolverPPPX::getCurrentSatNumber() const
