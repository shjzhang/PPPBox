#pragma ident "$Id: SolverPPP2.cpp 2645 2012-11-05 03:23:24Z shjzhang $"
...
namespace gpstk
{
   ...
   int SolverPPP2::classIndex = 9300000;


      // Returns an index identifying this object.
   int SolverPPP2::getIndex() const
   { return index; }


      // Returns a string identifying this object.
   std::string SolverPPP2::getClassName() const
   { return "SolverPPP2"; }
       ...
       */
   SolverPPP2::SolverPPP2(bool useNEU, bool fixPos)
   ...
   }  // End of 'SolverPPP2::SolverPPP2()'



      // Initializing method.
   void SolverPPP2::Init(void)
   {
       ...
       */
   gnssSatTypeValue& SolverPPP2::Process(gnssSatTypeValue& gData)
      throw(ProcessingException, SVNumException)
   {
   ...
   }  // End of method 'SolverPPP2::Process()'
       ...
       * @param gData     Data object holding the data.
       */
   gnssRinex& SolverPPP2::Process(gnssRinex& gData)
      throw(ProcessingException, SVNumException)
   {
       ...
       * @param gData    Data object holding the data.
       */
   gnssRinex& SolverPPP2::preCompute( gnssRinex& gData )
      throw(ProcessingException, SVNumException)
   {
...
//       cout << "numSat" << currSatSet.size() << endl;
         ...
         Vector<double> MW(gData.getVectorOfTypeID(TypeID::MWubbena));

         for( int i=0; i<numCurrentSV; i++ )
         {
            measVector( i                  ) = prefitC(i);
            measVector( i + 1*numCurrentSV ) = prefitL(i);
            measVector( i + 2*numCurrentSV ) = MW(i);
         }
...
//       cout << "measVector" << measVector << endl;
         ...
         if ( dummy.numSats() == numCurrentSV )
         {
            ...
            for( int i=0; i<numCurrentSV; i++ )
            {
               ...
               rMatrix( i + 2*numCurrentSV, i + 2*numCurrentSV ) 
                   = 25.0*wVec(i); // 1.0/((c*c+d*d)*varCode)
         ...
         for( SatIDSet::const_iterator itSat = currSatSet.begin();
              ...
              ++itSat )
         {
               // ambL1 are listed after ( srcIndexedTypes  )
            hMatrix( count1 + 1*numCurrentSV, numVar +                count1 ) = +0.106953378142;
               // ambWL are listed after ( srcIndexedTypes  + ambL1 )
            hMatrix( count1 + 2*numCurrentSV, numVar + numCurrentSV + count1 ) = +0.861918400322;
   ...
   }  // End of method 'SolverPPP2::preCompute()'
      ...
      //
   gnssRinex& SolverPPP2::Compute( gnssRinex& gData )
      throw(InvalidSolver)
   {
   ...
   }  // End of method 'SolverPPP2::Compute()'
      ...
      // @param gData    Data object holding the data.
      //
   int SolverPPP2::TimeUpdate( const Matrix<double>& phiMatrix,
...
of qMatrix");
   ...
   }  // End of method 'SolverPPP2::TimeUpdate()'
      ...
      // @param gData    Data object holding the data.
      //
   int SolverPPP2::MeasUpdate( const Vector<double>& prefitResiduals,
...
matrix and a priori state estimation vector do not match.");
   ...
   }  // End of method 'SolverPPP2::MeasUpdate()'
       ...
       * @param gData    Data object holding the data.
       */
   gnssRinex& SolverPPP2::postCompute( gnssRinex& gData )
...
//       cout << "var:WL" << covMatrix(numVar + 2*numCurrentSV + 1, numVar+2*numCurrentSV+1) << endl;
         ...
         for( int i=0; i<numCurrentSV; i++ )
         {
             BLC(i)    = solution(numVar                + i) ; 
             BWL(i)    = solution(numVar + numCurrentSV + i) ;
         }
         ...
         gData.insertTypeIDVector(TypeID::BLC, BLC);
            ...
            // Warning:
            // Store the widelane ambiguities as 'MWubbena'
         gData.insertTypeIDVector(TypeID::MWubbena, BWL);
...
//       cout << currSatSet.size() << endl;
   ...
   }  // End of method 'SolverPPP2::postCompute()'
       ...
       *
       */
   SolverPPP2& SolverPPP2::setNEU( bool useNEU )
   {
   ...
   }  // End of method 'SolverPPP2::setNEU()'
       ...
       *
       */
   SolverPPP2& SolverPPP2::setFixCoordinate( bool fixPos)
   {
   ...
   }  // End of method 'SolverPPP2::setFixCoordinate'
       ...
       */
   SolverPPP2& SolverPPP2::setCoordinatesModel( StochasticModel* pModel )
   {
   ...
   }  // End of method 'SolverPPP2::setCoordinatesModel()'


      /** Return the CURRENT number of satellite.
       */
   int SolverPPP2::getCurrentSatNumber() const
