#pragma ident "$Id: SolverRTKUC.hpp 2645 2011-06-08 03:23:24Z shjzhang $"

/**
 * @file SolverRTKUC.hpp
 * Class to compute the PPP Solution with ionospheric estimation.
 */

#ifndef GPSTK_SOLVERRTKUC_HPP
#define GPSTK_SOLVERRTKUC_HPP
...
namespace gpstk
{
       ...
       *
       *      // Declare a SolverRTKUC object
       *   SolverRTKUC pppSolver;
       *
       *     // PROCESSING PART
       *
       *   gnssRinex gRin;
       ...
       * @endcode
       *
       * The "SolverRTKUC" object will extract all the data it needs from the
       ...
       *
       * In any case, the "SolverRTKUC" object will also automatically add and
       ...
       *
       * If these weights are not assigned, then the "SolverRTKUC" object will
       ...
       *
       *
       * \warning "SolverRTKUC" is based on a Kalman filter, and Kalman filters
       ...
       *
       */
   class SolverRTKUC : public CodeKalmanSolver
   {
   public:
      ...
      virtual void AmbiguityUpdate(Vector<double>& state,
          ...
          *
          */
      virtual SolverRTKUC& setNEU( bool useNEU );
          ...
          * 'StochasticModel' (constant coordinates) or 'WhiteNoiseModel'.
          */
      virtual SolverRTKUC& setCoordinatesModel(StochasticModel* pModel);
          ...
          *
          */
      virtual SolverRTKUC& setIonosphericModel(StochasticModel* pModel);
          ...
          */
      SolverRTKUC& setInitialTropVar(double variance)
      { initialTropVar = variance; return (*this); };
          ...
          * @param variance      Initial variance assigned to this variable.
          */
      SolverRTKUC& setInitialIonoVar(double variance)
      ...
      virtual int getAmbFixedNumL1() const
         ...
         /** Return the CURRENT number of satellite.
          */
      virtual int getAmbFixedNumL2() const
         throw(InvalidRequest);
   ...
   private:
      ...
      virtual SolverRTKUC& setDefaultEqDefinition(
                             const gnssEquationDefinition& eqDef )
      { return (*this); };



   }; // End of class 'SolverRTKUC'

      //@}

}  // End of namespace gpstk
#endif   // GPSTK_SOLVERRTKUC_HPP
