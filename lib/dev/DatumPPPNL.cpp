#pragma ident "$Id: DatumPPPNL.cpp 2939 2012-04-06 19:55:11Z shjzhang $"
...
//  2012.10.20  Change this file name from "AmbDatumPos" to "DatumPPPNL" 
//
//============================================================================
...
namespace gpstk
{
   ...
   WhiteNoiseModel DatumPPPNL::whiteNoiseModel;
       ...
       *                  by SourceID.
       */
   DatumPPPNL& DatumPPPNL::Prepare( gnssRinex& gData )
   {
      ...
      if(currArcSet.find(oldArc) == currArcSet.end())
      {
               ...
               ++itamb )
          {
              ...
              if( std::abs( devOffVec(i) ) < 2*devSig )
              {
                  ...
                  cout << "newSat:" << devVec(i)  << " :" << newSat  << endl;
              }
      ...
      else
      {
         ...
         // Last, Insert datum ambiguity into 'ambFixedMap'
      cout << "newSat:" << newSat << endl;
   ...
   }  // End of method 'DatumPPPNL::Prepare()'


      /** 
       * Get the ambiguity datum set which are fixed directly.
       */
   std::map<SatID,double> DatumPPPNL::getAmbFixedMap( void )
      throw(InvalidDatumPPPNL)
   {
      ...
      if (!isPrepared)
      {
         GPSTK_THROW(InvalidDatumPPPNL("DatumPPPNL is not prepared"));
      }

         // Return
      return ambFixedMap;

   }  // End of method 'DatumPPPNL::getAmbFixedMap()'
