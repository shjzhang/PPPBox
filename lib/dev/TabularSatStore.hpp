         ...
         if(it1 == dtable.begin()) {
            ...
            if(exactMatch && nhalf==1) 
            {
               ++(it2 = it1);
                 ...
                 // add by shjzhang (tt-t1), (tt-t2), 2014/11/3.
               if(checkInterval && 
                  ( ( std::abs(it2->first - it1->first) > maxInterval ) ||
                    ( std::abs(ttag       - it1->first) > maxInterval ) ||
                    ( std::abs(ttag       - it2->first) > maxInterval ) ) ) 
               {
