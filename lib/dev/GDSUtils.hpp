...
//  Modifications
//
//  - Write/read the 'nominalPos' of the Source to file and from file.
//    2014.07.03 shjzhang
//
//  - Write/read the 'wetMap' of the Source to and from file.
//    2014.07.03 shjzhang
//
//============================================================================
...
//       : DataBin(){}
      ...
      { 
         ...
         StringBin nameBin(data.sourceName,s);
            ...
            // Now, we will save the nominal position into binary file
         DataBin<double> xnBin(data.nominalPos.X(),s);
         DataBin<double> ynBin(data.nominalPos.Y(),s);
         DataBin<double> znBin(data.nominalPos.Z(),s);

            // Now, save the bool, indicating the receiver's status
         DataBin<bool>  flagBin(data.staticFlag,s);


         std::map<TypeID, double> zwdMap(data.zwdMap);

            // zwdMap
         DataBin<size_t> sizeBin(zwdMap.size(),s);

         for( typeValueMap::const_iterator it = zwdMap.begin();
              it != zwdMap.end();
              ++it )
         {
      ...
      { 
         ...
         StringBin nameBin(s);
            ...
            // read nominal positoin from binary files
         DataBin<double> xnBin(s);
         DataBin<double> ynBin(s);
         DataBin<double> znBin(s);

            // read 'staticFlag' from binary file
         DataBin<bool> flagBin(s);


         data.type = typeBin.get();
         data.sourceName = nameBin.get();
            // nominal position
         data.nominalPos = Position(xnBin.get(),ynBin.get(),znBin.get());

            // flag 
         data.staticFlag = flagBin.get();

            // Read zwdMap from binary file
         data.zwdMap.clear();
         ...
         for(size_t i=0; i<sizeBin.get(); i++)
         {
            ...
            data.zwdMap[typeb.get()] = valueb.get();
         }
...
//    CommonTimeBin() : DataBin(){}
      ...
      CommonTimeBin(const CommonTime& time,std::ostream& s) 
         : DataBin(time) { write(s); }

      CommonTimeBin(std::istream& s) { read(s); }

      void write(std::ostream& s)
      { 
         CivilTime ctime(data);
         DataBin<int> yearBin(ctime.year,s);
         DataBin<int> monthBin(ctime.month,s);
         DataBin<int> dayBin(ctime.day,s);
         DataBin<int> hourBin(ctime.hour,s);
         DataBin<int> minuteBin(ctime.minute,s);
         DataBin<double> secondBin(ctime.second,s);

            // Get timeSystem
         TimeSystem timeSys = ctime.getTimeSystem();

            // Only print system in TimeSystem
         DataBin<TimeSystem::Systems> systemBin(timeSys.getTimeSystem(), s);
      }
      ...
      { 
         ...
         DataBin<TimeSystem::Systems> systemBin(s);

         int year   = yearBin.get();
         int month  = monthBin.get();
         int day    = dayBin.get();
         int hour   = hourBin.get();
         int min    = minuteBin.get();
         double sec = secondBin.get();

         TimeSystem timeSys( systemBin.get() );

         data = CivilTime(year, month, day, hour, min, sec, timeSys).convertToCommonTime();
...
//    gnssDataMapBin() : DataBin(){}
      ...
      void write(std::ostream& s)
      {
         ...
         for(it=data.begin();it!=data.end();it++)
         {
            CommonTimeBin timeBin(it->first,s);
      ...
      void read(std::istream& s)
      {
         ...
         for(size_t i=0; i<sizeBin.get(); i++)
         {
            CommonTimeBin timeBin(s);

            sourceDataMapBin sdMapBin(s);

            data.insert(std::pair<const CommonTime,sourceDataMap>(timeBin.get(),
                                                               sdMapBin.get()));
         }
      }
   };
   ...
   void dumpGnssDataMap(const gnssDataMap& gdsMap,const std::string& file)
   {
      ...
      while(!dataMap.empty())
      {
         ...
         CommonTime time(data.begin()->first);

         CivilTime ctime(time);

         s << StringUtils::leftJustify(ctime.printf(timeFormat),23) << " {" << endl;
         ...
         for(it=data.begin();it!=data.end();it++)
         {
                ...
                ++itsrc)
            {
               ...
               s << indentSpace <<"receiver: "<< source 
                                << " with zwd:"<< source.zwdMap[TypeID::wetTropo]
                                << " flag:"<< source.staticFlag
                                << " {" << endl;
   ...
   void testSave()
   {
      ...
      CivilTime  ctime(2010,12,31,0,0,0.0);
      CommonTime time0(ctime.convertToCommonTime());
