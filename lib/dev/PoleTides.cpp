...
namespace gpstk
{
      ...
      throw(InvalidRequest)
   {
      ...
      const CivilTime j2000(2000, 1, 1, 12, 0, 0.0);
         ...
         // Warning:
         // the conversion is omitted beacuse the affect on 
         // the pole tides is very small. 
         // In order to get more precise pole tide, you should
         // convert the time from 'GPS' to 'UTC'
      CommonTime UTC(t);

         // Change the 'TimeSystem' directly here.
      ...
      try
      {
         ...
         double ypbar(0.357 + timedif*0.00395);
            ...
            // Get polar motion from files (shjzhang)
         if(pEOPStore != NULL)
         {
            xdisp = (*pEOPStore).getXPole(UTC);
            ydisp = (*pEOPStore).getYPole(UTC);
         }
