#pragma ident "$Id: Equation.hpp 2575 2012-04-06 17:45:02Z shjzhang $"
...
//  Revision
//
//  2012/02/20     The structure of 'Equation' has been changed from 
//                 "gnssData<equationHeader, VariableSet>", to 
//                 "gnssData<equationHeader, VarCoeffMap>". 
//                 By shjzhang.
//
//  2014/02/20     Copy the changes from the old 'gpstk' to the version 2.3
//      
//============================================================================
...
namespace gpstk
{
   ...
   {
      inline std::string asString(const VarCoeffMap& vmap)
      {
         std::ostringstream oss;
         for( VarCoeffMap::const_iterator it = vmap.begin();
              it != vmap.end();
              ++it )
         {
            oss << (*it).first.getType() << "   "
                ...
                << (*it).first.getSatIndexed()<< std::endl;
         }
