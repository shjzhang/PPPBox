#pragma ident "$Id: SolverPODFB.hpp 1914 2009-05-24 22:09:30Z architest $"

/**
 * @file SolverPODFB.hpp
 * Class to compute the PPP solution in forwards-backwards mode.
 */

#ifndef GPSTK_SOLVERPODFB_HPP
#define GPSTK_SOLVERPODFB_HPP
...
namespace gpstk
{
       ...
       *
       * In reality, "SolverPODFB.hpp" objects are really "SolverPOD.hpp"
       * objects at their core, with some wrapper code that takes 'normal'
       * forwards input data, stores them, and feeds the internal "SolverPOD"
       ...
       *
       * In order to achieve this, SolverPODFB.hpp" objects work in three
       * distinct phases:
       *
       *    \li Initial "Process()" phase: This does a "normal" forward
       *        processing, like the "SolverPOD.hpp" objects. Data feed during
       ...
       *        final results.
       *
       * Take due note that the "SolverPODFB.hpp" class is designed to be used
       ...
       *
       *      // Declare a SolverPODFB object
       *   SolverPODFB pppSolver;
       *
       *     // PROCESSING PART
       *
       *   gnssRinex gRin;
       ...
       * @endcode
       *
       * The "SolverPODFB" object will also insert back postfit residual data
       ...
       *
       * You may configure the solver to work with a RAC system in the class
       * constructor or using the "setRAC()" method.
       *
       * In any case, the "SolverPODFB" object will also automatically add and
       ...
       *
       * If these weights are not assigned, then the "SolverPODFB" object will
       ...
       * @endcode
       *
       * \warning "SolverPODFB" is based on a Kalman filter, and Kalman filters
       ...
       * SolverPOD.hpp for base classes.
       *
       */
   class SolverPODFB : public SolverPOD
   {
   public:
      ...
      virtual bool LastProcess(gnssRinex& gData)
         ...
         /// Reset the ambiguity constraints for 'SolverPOD' 
      virtual void AmbiguityConstr( void );
      ...
      { return limitsCodeList; };
          ...
          */
      virtual SolverPODFB& setCodeList( std::list<double> codeList )
      ...
      { return limitsPhaseList; };
          ...
          * \warning Limits will be applied in the same order they were added.
          */
      virtual SolverPODFB& setPhaseList( std::list<double> phaseList )
      { limitsPhaseList = phaseList; return (*this); };
          ...
          * \warning Limits will be applied in the same order they were added.
          */
      virtual SolverPODFB& addPhaseLimit( double phaseLimit )
      { limitsPhaseList.push_back( phaseLimit ); return (*this); };


         /// Clears the list storing the limits for postfit residuals in phase.
      virtual SolverPODFB& clearPhaseList( void )
      { limitsPhaseList.clear(); return (*this); };


         /// Gets the minimum satellite arc size 
      virtual double getMinArcSize( void ) const
      { return minArcSize; };


         /** Sets the minimum satellite continuous arc size.
          *
          * @param minSize  minimum size for all continuous satellite arc
          */
      virtual SolverPODFB& setMinArcSize( double minSize)
      { minArcSize = minSize; return (*this); };
      ...
      { return rejectedMeasurements; };
          ...
          *
          */
      virtual SolverPODFB& setRAC( bool useRAC );
   ...
   private:
      ...
      std::list<gnssRinex> ObsData;
         ...
         /// List holding the information regarding every observation.
      std::list<gnssRinex> ObsDataBak;


         /// The a posterior ambiguity data
      ...
      std::list<double> limitsPhaseList;
         ...
         /// The minimum satellite tracking arc size
      double minArcSize;
      ...
      void checkLimits( gnssRinex& gData, double codeLimit, double phaseLimit );
         ...
         /// This method checks the satellite tracking size.
      void checkArcSize( gnssRinex& gData);
      ...
      virtual SolverPODFB& setDefaultEqDefinition(
                                       const gnssEquationDefinition& eqDef )
      { return (*this); };


      virtual SolverPODFB& Reset( const Vector<double>& newState,
                                  const Matrix<double>& newErrorCov )
      { return (*this); };


   }; // End of class 'SolverPODFB'
